...
$('#edit_categoria').on('change', function(){
    ...
    $.ajax({
        ...
        success: function(fabricantes){
            ...
            $('#edit_fabricante').html('<option selected disabled>Seleccionar Fabricante</option>');
            $('#edit_modelo').html('<option selected disabled>Seleccionar Modelo</option>');

            // Recorremos fabricantes y llenamos el select
            $.each(fabricantes, function(i, item){
                $('#edit_fabricante').append('<option value="'+item.id+'">'+item.fabricante.toUpperCase()+'</option>');
            });
        },
...
$('#edit_fabricante').on('change', function(){
    ...
    $.ajax({
        ...
        success: function(modelos){
            ...
            $.each(modelos, function(i, item){
                $('#edit_modelo').append('<option value="'+item.id+'">'+item.modelo.toUpperCase()+'</option>');
            });
        },
        error: function(xhr, status, error){
...
$('#edit_proveedor').on('change', function(){
    ...
    $.ajax({
        ...
        success: function(documentos){
            // Limpiamos selects dependientes
            $('#edit_documento').html('<option selected disabled>Seleccionar Documento</option>');

            $.each(documentos, function(i, item){
                $('#edit_documento').append('<option value="'+item.id+'">'+item.documento.toUpperCase()+'</option>');
            });
        },
        error: function(xhr, status, error){
